...
def load_fjs(lines, num_mas, num_opes):
    ...
    '''
    flag = 0
    matrix_proc_time = np.zeros(shape=(num_opes, num_mas))
    # matrix_pre_proc = np.full(size=(num_opes, num_opes), dtype=torch.bool, fill_value=False)
    ...
    matrix_proc_time = matrix_proc_time[np.newaxis, :]
    nums_ope = np.array(nums_ope)
    return matrix_proc_time, nums_ope
